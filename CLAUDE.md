...
Connection methods — Infinite login + Hosted Auth + extension (equal-option chooser)

The connect UI (/accounts, /settings/accounts) is an equal-option chooser; users pick whichever fits. All persist encrypted-at-rest material and validate with the provider before saving.

Infinite login (RECOMMENDED — the "stays connected" method; CredentialsReconnectCapable adapter capability): email + password + the authenticator-app TOTP shared secret. Unipile logs the account in inside a stable, region-matched residential-proxy session and, when the session later drops (Unipile CREDENTIALS status → our restricted/disconnected inbound event), orchestration SILENTLY re-authenticates using the stored credentials, generating the current TOTP from the stored secret to solve LinkedIn's 2FA checkpoint — no user action (AccountsService.attemptInfiniteReconnectByRef, triggered from InboundEventsService). Requires authenticator-app 2FA (not SMS). This is the same mechanism HeyReach's "Infinite Login" uses. NOTE (reversed decision): earlier this credentials path was removed for fear of datacenter-login logout loops; that concern is resolved by the region-matched residential proxy (which we already give every account) + the stored TOTP for automatic re-auth, so credentials are re-introduced AS Infinite login. TOTP generation is RFC 6238 in packages/adapters/src/unipile/totp.ts (zero-dep); the password + TOTP secret are stored AES-256-GCM in sending_account_secrets, never logged. Mock-safe: MockChannelAdapter implements connect + reconnectWithCredentials + simulateCredentialsDisconnect.
Hosted Auth (lowest friction, no password): Unipile-hosted login (HostedAuthCapable adapter capability). The user clicks Connect, logs into LinkedIn once on the provider's hosted page (provider handles 2FA/checkpoints in a consistent browser+proxy context — the most logout-resistant path), and the provider calls our webhook (POST /webhooks/hosted/unipile) with a one-time token (account_link_requests) to finalize. No password, no cookie, no extension. ADAPTER=mock simulates the callback via GET /dev/hosted-auth/complete so the flow is testable locally.
Browser extension ("connect another way"): the 10xConnect MV3 extension (apps/extension) captures the user's real li_at + matching user-agent and connects via POST /accounts/connect. Manual li_at paste is a dev-only testing toggle.
Multiple LinkedIn accounts per workspace, capped by the billing slot count (subscriptions.slot_count; ALLOW_UNLIMITED_ACCOUNTS bypasses for dev/self-host). Connecting a NEW account consumes a slot (AccountsService.assertSlotAvailable); a reconnect targets a SPECIFIC account (reconnectAccountId) and refreshes its session/provider handle in place, preserving campaign state, and never consumes a slot. Each account keeps its own dedicated proxy + per-account rate governor, so ban risk stays isolated per account.
...
sending_accounts — id, workspace_id, type (linkedin|mailbox), label, connection_method (extension | cookie | credentials [Infinite login] | hosted_auth), proxy (bundled|own + region), location, status (active|warming|paused|restricted|disconnected), health_score, warmup_state, avatar_url. Multiple type='linkedin' rows per workspace, capped by subscriptions.slot_count (the one-per-workspace unique index was dropped in 20260702130000_multi_account.sql). account_link_requests carries reconnect_account_id so a hosted-auth reconnect finalizes the correct row.
